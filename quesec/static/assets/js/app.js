...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // mega menu active class
    const megaMenu = document.querySelectorAll('.mega-menu-link a');
    if (megaMenu) {
        megaMenu.forEach(menu => {
            if (menu.href === currentUrl) {
                menu.classList.add('active');
                let parentMenuItem = menu.closest('.mega-menu-item');
                if (parentMenuItem) {
                    if (parentMenuItem.parentElement.closest(".menu-item")) {
                        parentMenuItem.parentElement.closest(".menu-item").classList.add('active')
                    }
        ...
        if (window.matchMedia("(max-width: 991px)").matches) {
            ...
            menuLinks.forEach(function (link) {
                ...
                if (megaMenu) {
                    ...
                    const megaMenuItems = megaMenu.querySelectorAll(".mega-menu-item .menu-title-wrapper");
                    megaMenuItems.forEach(item => {

                        const megaSubMenu = item.parentElement.querySelector(".mega-sub-menu");
    ...
    // custom select
    const selectContainer = document.querySelector('.language-option');
    if (selectContainer) {
    ...
    const popupInterval = 10 * 60 * 1000; // 1 minutes in milliseconds
    const newsletterPopup = document.querySelector('.newsletter-modal')
    const newsletterCloseBtn = document.querySelector('.newsletter-close-btn')
    const newsletterOverlay = document.querySelector('.popup-overlay')
    ...
    const shouldShowPopup = () => {
        const lastClosedTime = localStorage.getItem('newsletterPopupClosed');
    ...
    // product color change option
    const hasColorOptionItems = document.querySelectorAll('.has-color-option')
    hasColorOptionItems.forEach(card => {
    ...
    // single wishlist select btn
    const singleWishlistBtn = document.querySelectorAll('.single-wishlist-btn')
    if (singleWishlistBtn) {
        singleWishlistBtn.forEach(wishlistBtn => {
    ...
    // side menu functions
    const sideMenuBtn = document.querySelector('.side-menu-open-btn');
    ...
    // image zoomer
    let imageZoomer = (zoomableContainers, minScale = 2, maxScale = 5, enableWheelZoom = true, bgColor = "rgb(255, 255, 255)") => {
    ...
    function activateTab(tabData) {
        ...
        document.querySelector(`.tab-btn[data-tab="${tabData}"]`).classList.add("active");
    ...
    const ratings = {
        ...
        4: 1, // 1 person gave 4 stars
        3: 0, // 0 people gave 3 stars
    ...
    document.querySelectorAll('.rating-row').forEach((row, index) => {
        const starLevel = 5 - index; // Starting from 5 stars, going down
        const count = ratings[starLevel];

        // Calculate the percentage for the current star level
        const percentage = (count / totalRatings) * 100 || 0;

        // Update the filled bar width
        row.querySelector('.filled-bar').style.width = `${percentage}%`;
    ...
    ratingInputs.forEach(input => {
        input.addEventListener('change', () => {
            ...
            ratingDisplay.textContent = `You selected ${selectedValue} star(s)`;
        });
    });

    // img input controller
    let uploadBtn = document.querySelector('.file-upload-wrapper');
    if (uploadBtn) {
        let isRemoveButtonClicked = false;
        uploadBtn.addEventListener('click', function () {
            // Only open file dialog if remove button was not clicked
            if (!isRemoveButtonClicked) {
                let inputFile = uploadBtn.querySelector('.file-upload-input');
                inputFile.click();
            }
        ...
        // uploadBtn.addEventListener('click', function () {
        let previewImg = uploadBtn.querySelector('.file-upload-preview');
        let inputFile = uploadBtn.querySelector('.file-upload-input')
        ...
        }, { once: true }); // Add { once: true } to ensure this event runs only once
    }
    ...
    const cartOverlay = document.querySelector('.cart-box-overlay');
    if (cartBtn && cartBox) {
    ...
    const noteInput = document.querySelector('.note-form-wrapper');
    const noteCancelBtn = document.querySelector('.note-cancel-btn');
    if (addNoteBtn) {
    ...
    // gift wrapper
    const giftWrapperBtn = document.querySelector('.add-gift-wrap');
    const giftWrapper = document.querySelector('.gift-form-wrapper');
    const giftWrapperCancelBtn = document.querySelector('.gift-cancel-btn');
    if (giftWrapperBtn) {
    ...
    // input toggle btns
    const inputToggleBtns = document.querySelectorAll('.input-info-btn'); // Select all radio buttons in the group
    const inputInfoForms = document.querySelectorAll('.input-info'); // Select all info forms

    if (inputToggleBtns && inputInfoForms) {
        inputToggleBtns.forEach((toggleBtn, index) => {
            toggleBtn.addEventListener('change', function () {
                inputInfoForms.forEach((form, formIndex) => {
                    if (index === formIndex && this.checked) {
    ...
    function setupCustomButton(customButtonSelector, targetTabNumber) {
        const customButton = document.querySelector(customButtonSelector);

        if (customButton) {
            customButton.addEventListener("click", () => {
                activateTab(targetTabNumber);
                document.querySelector(`.tab-btn[data-tab="${targetTabNumber}"]`).scrollIntoView({ behavior: 'smooth' });
            });
        }
    }
    ...
    if (copyButton && shareLinkInput) {
        ...
        copyButton.addEventListener('click', (e) => {
            navigator.clipboard.writeText(url).then(() => {
                ...
                /* eslint-disable */console.error(...oo_tx(`1046851690_818_16_818_54_11`,'Failed to copy: ', err));
            });
            setTimeout(() => {
    ...
    // aks question
    const aksQuestion = document.querySelector('.aks-question-btn');
    if (aksQuestion) {
    ...
    if (popups) {
        popups.forEach((popup) => {
            const closeBtn = popup.querySelector('.popup-overlay');
